...
<article id="post-<?php the_ID(); ?>" <?php post_class('blog-box mb-50px pformat-standard fl-wrap fw-post tranform-main-img'); ?>>
	<div class="post-itemt-wrap">
		...
		<?php } elseif (has_post_thumbnail( )){ ?>
	    	<?php
	    		$link_video = get_post_meta(get_the_ID(), '_cth_embed_video', true);
        		if(get_post_meta(get_the_ID(), '_cth_embed_video', true)!=""){  ?>
					<div class="o-hidden post-video-wrap img-video p-relative fl-wrap tranform-img">
			        	<?php the_post_thumbnail('pxaas-featured-image',array('class'=>'transition-4 respimg') ); ?>
			        	<a class="play-trigger p-absolute fs-30 pl-5px color-fff bg-green bg-fff-hvr color-green-hvr radius-50 text-center transition-2" href="<?php echo esc_url( $link_video ); ?>" data-lity=""><i class="fa fa-play"></i></a>
			    	</div>
    		<?php }else { ?>
