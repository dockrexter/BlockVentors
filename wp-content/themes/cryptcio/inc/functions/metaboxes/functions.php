<?php 
require_once(CRYPTCIO_METABOXES . '/default_options.php');
require_once(CRYPTCIO_METABOXES . '/page.php');
require_once(CRYPTCIO_METABOXES . '/post.php');
if (class_exists('Woocommerce')) {
    require_once(CRYPTCIO_METABOXES . '/products.php');
}
require_once(CRYPTCIO_METABOXES . '/gallery.php');
require_once(CRYPTCIO_METABOXES . '/related_post.php');
require_once(CRYPTCIO_METABOXES . '/service.php');
require_once(CRYPTCIO_METABOXES . '/events.php');
require_once(CRYPTCIO_METABOXES . '/user-profile.php');
function cryptcio_post_gallery_related(){
    ...
    <?php if (is_array($gallery_related)) : ?>
        <?php if (count($gallery_related) > 0) : ?>
            <div class="mad_section_bg1 gallery_related">
                <h2 class="align_center"><?php echo esc_html__('Related Projects','cryptcio' );?></h2>
                        ...
                        <div class="owl-carousel" data-max-items="4">
                            ...
                            <div>
                                <div class="item_wrapper">
                                    ...
                                    <figure>
                                        <?php $image = get_the_post_thumbnail($entry, 'cryptcio_gallery'); 
...
function cryptcio_sidebars() {
    global $wp_registered_sidebars;

    $sidebar_options = array();
    $sidebar_options['default'] = esc_html__('Default sidebar', 'cryptcio');
    $sidebar_options['none'] = esc_html__('None', 'cryptcio');
...
function cryptcio_metabox_template($meta_boxes) {
    global $post;
    $output = '';
    ob_start();
    foreach ($meta_boxes as $meta_box):
                ...
                <div class="metainner">
                    <div class="box-option">
                        <label for='upload_image'>
                            ...
                            <br/>
                            <input class="button_upload_image button" id="<?php echo esc_attr($name) ?>" type="button" value="<?php echo esc_html__('Upload File', 'cryptcio') ?>" />&nbsp;
                            <input class="button_remove_image button" id="<?php echo esc_attr($name) ?>" type="button" value="<?php echo esc_html__('Remove File', 'cryptcio') ?>" />
                        </label>
                    </div>
                </div>
            </div>
        ...
        <?php if ($type == "color") : // color ?>
            <div class="metabox" <?php echo esc_attr($required); ?>>
                <h3><?php echo esc_html($title) ?></h3>
                <div class="metainner">
                    <div class="box-option cryptcio-meta-color">
                        <input type="text" id="<?php echo esc_attr($name) ?>" name="<?php echo esc_attr($name) ?>" value="<?php echo stripslashes($meta_box_value) ?>" size="50%" class="cryptcio-color-field" />
                        <label class="cryptcio-transparency-check" for="<?php echo esc_attr($name) ?>-transparency"><input type="checkbox" value="1" id="<?php echo esc_attr($name) ?>-transparency" class="checkbox cryptcio-color-transparency"<?php if ($meta_box_value == 'transparent') echo ' checked="checked"' ?>> <?php echo esc_html__('Transparent', 'cryptcio') ?></label>
                    </div>
                    <div class="box-info"><label for="<?php echo esc_attr($name) ?>"><?php echo esc_html($desc) ?></label></div>
                </div>
            </div>
...
function cryptcio_show_meta_box($meta_boxes) {
    if (count($meta_boxes)) :
        $metabox_template = cryptcio_metabox_template($meta_boxes);
        echo '<div class="postoptions">'.$metabox_template.'</div>'; //end div class postoptions
    endif;
}

function cryptcio_save_meta_data($post_id, $meta_boxes) {
    global $post;
...
function cryptcio_use_default_meta() {
    global $wp_query;
...
function cryptcio_get_meta_value($meta_key, $boolean = false) {
    global $wp_query, $cryptcio_settings;
    ...
    if (is_category()) {
        ...
        if (is_singular()) {
            ...
            if (!is_home() && is_front_page()) {
                if (isset($cryptcio_settings[$meta_key]))
                    $value = $cryptcio_settings[$meta_key];
            } else if (is_home() && !is_front_page()) {

                if (isset($cryptcio_settings['blog-'.$meta_key])){
                    $value = $cryptcio_settings['blog-'.$meta_key];
                }else{
...
function cryptcio_show_tax_add_meta_boxes($meta_boxes) {
    if (!isset($meta_boxes) || empty($meta_boxes))
        return;

    foreach ($meta_boxes as $meta_box) {
        cryptcio_show_tax_add_meta_box($meta_box);
    }
}

// Show Taxonomy Add Meta Box
function cryptcio_show_tax_add_meta_box($meta_box) {
    ...
    if ($type == "upload") : // upload image ?>
        <div class="form-field">
            ...
            <label for='upload_image'>
                ...
                <button class="button_upload_image button" id="<?php echo esc_attr($name) ?>"><?php echo esc_html__('Upload Image', 'cryptcio') ?></button>
                <button class="button_remove_image button" id="<?php echo esc_attr($name) ?>"><?php echo esc_html__('Remove Image', 'cryptcio') ?></button>
            </label>
            <?php if ($desc) : ?><p><?php echo esc_html($desc) ?></p><?php endif; ?>
        </div>
    <?php endif; 
...
// Show Taxonomy Add Meta Boxes
function cryptcio_show_tax_edit_meta_boxes($tag, $taxonomy, $meta_boxes) {
    if (!isset($meta_boxes) || empty($meta_boxes))
        return;

    foreach ($meta_boxes as $meta_box) {
        cryptcio_show_tax_edit_meta_box($tag, $taxonomy, $meta_box);
    }
}

// Show Taxonomy Add Meta Box
function cryptcio_show_tax_edit_meta_box($tag, $taxonomy, $meta_box) {
...
// Save Tax Data
function cryptcio_save_taxdata( $term_id, $tt_id, $taxonomy, $meta_boxes ) {
    if (!isset($meta_boxes) || empty($meta_boxes))
        return;
...
// Create Meta Table
function cryptcio_meta_page_assets(){
    wp_enqueue_script( 'cryptcio-metabox', get_template_directory_uri() . '/inc/functions/metaboxes/js/metabox.js', array('jquery'), CRYPTCIO_VERSION, true); 
    wp_enqueue_style("cryptcio-page-metabox-style",get_template_directory_uri().'/inc/functions/metaboxes/css/metabox.css?ver=' . CRYPTCIO_VERSION);
}
add_action('admin_enqueue_scripts', 'cryptcio_meta_page_assets' );


function cryptcio_get_post_banner_block(){
    global $post, $cryptcio_settings,$wp_query;
    $cat = $wp_query->get_queried_object();
    $static = "";  
    if(isset($cryptcio_settings['top_banner_block']) && $cryptcio_settings['top_banner_block'] != '') {
        $static = $cryptcio_settings['top_banner_block'];  
    }
    if($static != ''){      
        ...
        if (class_exists('Ultimate_VC_Addons')) {
            ...
            $cryptcio_ult_assets = str_replace('js/', '', $uvc_addons->assets_js);
            $isAjax = false;
            ...
            wp_register_script('ultimate-script', $cryptcio_ult_assets . 'min-js/ultimate.min.js', array('jquery'), ULTIMATE_VERSION, false);
            wp_register_script('ultimate-appear', $cryptcio_ult_assets . 'min-js/jquery.appear.min.js', array('jquery'), ULTIMATE_VERSION);
            wp_register_script('ultimate-custom', $cryptcio_ult_assets . 'min-js/custom.min.js', array('jquery'), ULTIMATE_VERSION);
            wp_register_script('ultimate-vc-params', $cryptcio_ult_assets . 'min-js/ultimate-params.min.js', array('jquery'), ULTIMATE_VERSION);

            // register css
            wp_register_style('ultimate-animate', $cryptcio_ult_assets . 'min-css/animate.min.css', array(), ULTIMATE_VERSION);
            wp_register_style('ultimate-style', $cryptcio_ult_assets . 'min-css/style.min.css', array(), ULTIMATE_VERSION);
            wp_register_style('ultimate-style-min', $cryptcio_ult_assets . 'min-css/ultimate.min.css', array(), ULTIMATE_VERSION);
            ...
            else if($ultimate_js == 'disable')
            {
                ...
                if( stripos( $post_content, '[ult_countdown') ) {
                    wp_enqueue_script('jquery.timecryptcio');
            ...
            if($ultimate_css == "enable"){
                ...
                //if( stripos( $post_content, '[ultimate_carousel') ) {
                //  wp_enqueue_style("ult-slick", $cryptcio_ult_assets . 'slick/slick.css');
                //  wp_enqueue_style("ult-icons", $cryptcio_ult_assets . 'slick/icons.css');
                //}
            } else {
                ...
                if( stripos( $post_content, '[icon_counter') ) {
                    wp_enqueue_style('ultimate-animate');
                    wp_enqueue_style('ultimate-style');
                    wp_enqueue_style('aio-flip-style', $cryptcio_ult_assets . 'min-css/flip-box.min.css');
                }
                if( stripos( $post_content, '[ult_countdown') ) {
                    wp_enqueue_style('countdown_shortcode', $cryptcio_ult_assets . 'min-css/countdown.min.css');
                }
                if( stripos( $post_content, '[ultimate_icon_list') ) {
                    wp_enqueue_style('ultimate-animate');
                    wp_enqueue_style('aio-tooltip', $cryptcio_ult_assets . 'min-css/tooltip.min.css');
                }
                if( stripos( $post_content, '[ultimate_carousel') ) {
                    wp_enqueue_style("ult-slick", $cryptcio_ult_assets . 'slick/slick.css');
                    wp_enqueue_style("ult-icons", $cryptcio_ult_assets . 'slick/icons.css');
                ...
                if( stripos( $post_content, '[ult_buttons') ) {
                    wp_enqueue_style( 'ult-btn', $cryptcio_ult_assets . 'min-css/btn-min.css' );
                }
                if( stripos( $post_content, '[ultimate_heading') ) {
                ...
                if( stripos( $post_content, '[bsf-info-box') ) {
                    wp_enqueue_style('ultimate-animate');
                    wp_enqueue_style('info-box-style', $cryptcio_ult_assets . 'min-css/info-box.min.css');
                }
                if( stripos( $post_content, '[info_cryptcio') ) {
                    wp_enqueue_style('ultimate-animate');
                    wp_enqueue_style('info-cryptcio', $cryptcio_ult_assets . 'min-css/info-cryptcio.min.css');
                }
                if( stripos( $post_content, '[ultimate_info_banner') ) {
                ...
                if( stripos( $post_content, '[icon_timeline') ) {
                    wp_enqueue_style('ultimate-animate');
                    wp_enqueue_style('aio-timeline', $cryptcio_ult_assets . 'min-css/timeline.min.css');
                }
                ...
                if( stripos( $post_content, '[interactive_banner_2') ) {
                    wp_enqueue_style('utl-ib2-style', $cryptcio_ult_assets . 'min-css/ib2-style.min.css');
                }
                if( stripos( $post_content, '[interactive_banner') ) {
                    wp_enqueue_style('ultimate-animate');
                    wp_enqueue_style('aio-interactive-styles', $cryptcio_ult_assets . 'min-css/interactive-styles.min.css');
                }
                ...
                if( stripos( $post_content, '[ultimate_modal') ) {
                    wp_enqueue_style('ultimate-animate');
                    wp_enqueue_style('ultimate-modal', $cryptcio_ult_assets . 'min-css/modal.min.css');
                }
                if( stripos( $post_content, '[ultimate_info_table') ) {
                    wp_enqueue_style('ultimate-animate');
                    wp_enqueue_style("ultimate-pricing", $cryptcio_ult_assets . 'min-css/pricing.min.css');
                }
                ...
                if( stripos( $post_content, '[swatch_container') ) {
                    wp_enqueue_style('swatchbook-css', $cryptcio_ult_assets . 'min-css/swatchbook.min.css');
                }
                if( stripos( $post_content, '[stat_counter') ) {
                    wp_enqueue_style('ultimate-animate');
                    wp_enqueue_style('stats-counter-style', $cryptcio_ult_assets . 'min-css/stats-counter.min.css');
                }
                if( stripos( $post_content, '[ultimate_video_banner') ) {
            ...
            wp_register_script('ultimate-appear', $cryptcio_ult_assets . 'min-js/jquery.appear.min.js', array('jquery'), ULTIMATE_VERSION, true);
            wp_register_script('ultimate-custom', $cryptcio_ult_assets . 'min-js/custom.min.js', $dependancy, ULTIMATE_VERSION, true);
            wp_register_script('ultimate-smooth-scroll', $cryptcio_ult_assets . 'js/SmoothScroll.js', array('jquery'), ULTIMATE_VERSION, true);
            ...
            if(function_exists('vc_is_editor')){
                if(vc_is_editor()){
                    wp_enqueue_style('vc-fronteditor', $cryptcio_ult_assets . 'min-css/vc-fronteditor.min.css');
                }
            }
