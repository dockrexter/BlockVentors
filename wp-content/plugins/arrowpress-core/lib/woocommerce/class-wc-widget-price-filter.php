 ...
 * @extends  WC_Widget
 */
class CryptoCio_WC_Widget_Price_Filter extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_name        = __( 'Filter Products Custom by Price', 'woocommerce' );
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',
	...
	public function widget( $args, $instance ) {
		...
		if(! is_post_type_archive( 'product' ) && ! is_tax( get_object_taxonomies( 'product' ) ) ){
			if ( strpos(get_the_content(), 'arrowpress_search') === false) {
				return;
			}
		}
		...
		echo '<form method="get" action="' . esc_url( $form_action ) . '">
			<div class="price_slider_wrapper">
				...
				<div class="price_slider_amount">
                    ...
                    <button type="submit" class="button btn-default">'.'<span>'. esc_html__( 'Filter', 'arrowpress-core' ) .'</span>'.'</button>
