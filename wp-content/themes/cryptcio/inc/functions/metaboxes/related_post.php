<?php
if ( ! class_exists( 'cryptcio_Related_Posts' ) ) :

class cryptcio_Related_Posts {
    ...
    function __construct(){
        ...
        $this->supported_post_types = array( 'gallery','service' );
        ...
        add_action( 'admin_enqueue_scripts', array( $this, 'cryptcio_related_metabox_enqueues' ) );
        add_action( 'add_meta_boxes', array( $this, 'cryptcio_related_add_metabox' ) );
        add_action( 'save_post', array( $this, 'cryptcio_related_metabox_save' ) );
    }

    function cryptcio_related_metabox_enqueues( $screen ) {
    ...
    function cryptcio_related_add_metabox( $post_type ) {
        if ( in_array( $post_type, $this->supported_post_types ) ) {
            add_meta_box(
                'related-metabox',
                esc_html__( 'Related post', 'cryptcio' ),
                array( $this, 'cryptcio_related_metabox_fields' ),
                $post_type,
                'side',
                'default'
            );
    ...
    function cryptcio_related_metabox_fields( $post ) {
        ...
        $post_obj_array = $query_posts->query(
            array(
                'post_type'         => array('gallery','service'),
                'post_status'       => 'publish',
                'pagination'        => false,
                'posts_per_page'    => '-1',
        ...
        <select id="related-post-select" name="related-post-ids[]" multiple="multiple"><?php
            ...
            <option value="<?php echo esc_attr($post_obj->ID); ?>"<?php echo ( in_array( $post_obj->ID, $post_ids) ) ? ' selected="selected"' : ''; ?>><?php echo ''.$post_obj->post_title; ?></option>
            <?php
            endforeach; ?>
        </select><?php
        endif; 
    ...
    function cryptcio_related_metabox_save( $post_id ) {
...
if ( is_admin() ) {
    add_action( 'load-post.php', 'cryptcio_sn_related_init' );
    add_action( 'load-post-new.php', 'cryptcio_sn_related_init' );
}

function cryptcio_sn_related_init() {
    new cryptcio_Related_Posts();
}
