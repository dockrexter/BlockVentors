...
function cryptcio_like_scripts() {
...
add_action('init', 'cryptcio_like_scripts');

add_action('wp_ajax_nopriv_cryptcio-post-like', 'cryptcio_post_like');
add_action('wp_ajax_cryptcio-post-like', 'cryptcio_post_like');

function cryptcio_post_like() {
    $nonce = $_POST['nonce'];
    if (!wp_verify_nonce($nonce, 'ajax-nonce'))
        die('Nope!');

    if (isset($_POST['cryptcio_post_like'])) {
        $like_type = 'post';
        ...
        if (is_user_logged_in()) { // user is logged in
            ...
            if (!cryptcio_AlreadyLiked($post_id)) { // like the post
                if($like_type == 'comment') {
...
function cryptcio_AlreadyLiked($post_id) { // test if user liked before
    $like_type = 'post';
    if (isset($_POST['like_type']) && !empty($_POST['like_type'])) {
        $like_type = $_POST['like_type'];
    }
...
function cryptcio_getPostLikeLink($post_id, $like_type = 'post') {
    if ($like_type == 'comment') {
    ...
    if (cryptcio_AlreadyLiked($post_id)) {
    ...
    $output = '<a href="#" class="cryptcio-post-like' . $class . '"  data-like_type="'.$like_type.'" data-post_id="' . $post_id . '" title="' . $title . '">' . $heart . $count . $like_text .'</a>';
    return $output;
}
